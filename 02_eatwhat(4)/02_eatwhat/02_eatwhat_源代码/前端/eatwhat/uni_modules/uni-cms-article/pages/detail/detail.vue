  ...
  <unicloud-db v-slot:default="{data, loading, error, options}" :collection="collection" :options="formData"
    ...
    <template v-if="!loading && data">
      ...
      <view class="meta">
		...
		<view class="excerpt" v-for="(item, index) in data.content.ops" :key="index">
		  <text class="text">{{ item.insert}}</text>
		</view>
        <view class="excerpt">
...
export default {
  ...
  methods: {
	 tj() {
	   const db = uniCloud.database();
	   db.collection('sc').add({ ccsc: this.id, yhm: getApp().globalData.yhm })
	     .then(res => {
