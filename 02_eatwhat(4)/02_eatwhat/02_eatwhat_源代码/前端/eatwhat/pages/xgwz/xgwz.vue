<template>
	...
	<view>
		<view class="box-bg">
			...
			<!--  #endif -->
			<!-- #ifdef H5 -->
			<uni-nav-bar shadow left-icon="arrow-left" leftText="返回" @clickLeft="fh()" title="修改文章" />
			<!--  #endif -->
		</view>
		<view class="xb1">
		...
		<view class="tx1">
			...
			<uni-easyinput class="input1" v-model="text" type="textarea" autoHeight maxlength="-1"
				placeholder="请输入文字"></uni-easyinput>
		</view>
		<button class="button1" type="button" @click="sc()">发布修改内容</button>
	</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad() {
			this.wz0 = getApp().globalData.wz0
			this.bt = getApp().globalData.wz1
			this.text = getApp().globalData.wz2
			this.fm = getApp().globalData.wz4
		},
		methods: {
			// 新增图片
			uploadAvatarImg() {
			...
			uploadFile() {
				uni.chooseImage({
					success: (chooseImageRes) => {
						...
						uni.uploadFile({
							url: config.api+"/upload", //仅为示例，非真实的接口地址
							method: 'POST',
							header: {
